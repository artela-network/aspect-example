...
const tokenBin = fs.readFileSync('./contract/wasm_contracts_token_sol_ArtToken.bin', "utf-8");
const tokenTarget = fs.readFileSync('./contract/wasm_contracts_token_sol_ArtToken.abi',"utf-8")
...
const brokerBin = fs.readFileSync('./contract/wasm_contracts_broker_sol_Borker.bin', "utf-8");
const brokerTarget = fs.readFileSync('./contract/wasm_contracts_broker_sol_Borker.abi',"utf-8")
...
async function f() {
    ...
    let token_instance = token_contract.deploy({"arguments":[10000000000]}).send({from: tokenDeployer, nonce: nonceValtokenDeployer});
    let contractAddress="";
   ...
   let instance = aspect.deploy({
        data: '0x' + aspectCode,
        properties: [{'key': 'ScheduleTo', 'value': brokerAddress},{'key': 'Broker', 'value': brokerDeployer },{'key': 'binding', 'value': brokerAddress},{'key': 'owner', 'value':AspectDeployer }]
    ...
    await token_contract.methods.transfer(brokerDeployer,1000000000)
    ...
    for (;;){
        ...
        let result= await token_contract.methods.balanceOf(targetAccount).call({from: tokenDeployer, nonce: nonceValAspectDeployer + nonceskip})
