...
export class Aspect implements IAspectTransaction, IAspectBlock, IAspectOperation, ITransactionVerifier {
    ...
    operation(ctx: OperationCtx, data: Uint8Array): Uint8Array{
        ...
        const sessionKey = sys.utils.uint8ArrayToHex(data);
        const stateVal = ctx.mutableState.get<string>(sessionKey);
        stateVal.set(ctx.tx.content.unwrap().from);
        return new Uint8Array(0);
    }

    verifyTx(ctx: VerifyTxCtx, validationData: Uint8Array): Uint8Array {
        // looks up session key -> account mapping
        const sessionKey = sys.utils.uint8ArrayToHex(validationData);

        // retrieve account and check if it matches
        const address = ctx.state.get<string>(sessionKey).unwrap();
        sys.require(address != '', 'invalid session key');

        return sys.utils.hexToUint8Array(address);
    }
}
